...
export default function SocialFloatingButtons() {
  const socialLinks = [
    ...
    {
      icon: WhatsAppIcon,
      href: 'https://wa.me/34722350678?text=Hola%20José,%20me%20interesa%20conocer%20más%20sobre%20tus%20servicios%20de%20Data%20Engineering',
      label: 'WhatsApp',
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 hidden md:flex flex-col items-end space-y-3">
      {/* Social Buttons - Simplificados */}
        ...
        {socialLinks.map((social, index) => (
          <div key={social.label} className="relative group">
            ...
            <div className="absolute right-full mr-3 top-1/2 -translate-y-1/2 
                         bg-gray-900/95 text-white text-sm py-2 px-3 rounded-lg
              ...
              <div className="absolute left-full top-1/2 -translate-y-1/2 
                           border-4 border-transparent border-l-gray-900/95" />
            </div>

            {/* Indicador de disponibilidad solo en LinkedIn */}
            {index === 0 && (
              <div className="absolute -top-1 -right-1 h-3 w-3 bg-green-500 rounded-full 
                           shadow-sm border-2 border-white animate-pulse" />
            )}
          </div>
        ))}
      </div>
